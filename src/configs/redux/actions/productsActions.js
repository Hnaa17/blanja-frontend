...
export const getProduct = ({ page, limit, search, sort}) => async (dispatch) => {
    try {
        const navigate = useNavigate();
        dispatch({type: ActionTypes.GET_PRODUCT_PENDING});
        const { data } = await axio({
            url: `${process.env.REACT_APP_BACKEND}products?&page=${page}&limit=${limit}${search ? "&search=" + search : ""}${sort ? "&sort=" + sort : ""}`,
            method: "GET",
        });
...
export const setDataProduct = (page, type) => (dispatch) => {
    axios
      .get(
        `${process.env.REACT_APP_BACKEND}products?page=${page}&type=${type}`
      )
      .then((result) => {
